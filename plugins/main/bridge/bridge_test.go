...
var _ = Describe("bridge Operations", func() {
	...
	It("creates a bridge", func() {
		...
		err := originalNS.Do(func(ns.NetNS) error {
			...
			bridge, err := setupBridge(netops, conf)
			Expect(err).NotTo(HaveOccurred())
	...
	It("handles an existing bridge", func() {
		...
		err := originalNS.Do(func(ns.NetNS) error {
			...
			err := netops.LinkAdd(&netlink.Bridge{
    ...
    "ipam": {
		...
		err = originalNS.Do(func(ns.NetNS) error {
			...
			link, err := netops.LinkByName(BRNAME)
			Expect(err).NotTo(HaveOccurred())
			Expect(link.Attrs().Name).To(Equal(BRNAME))

			// Ensure bridge has gateway address
			addrs, err := netops.AddrList(link, syscall.AF_INET)
			Expect(err).NotTo(HaveOccurred())
