...
func (c *chain) setup(ipt *iptables.IPTables, rules [][]string) error {
	...
	for _, rule := range rules {
		if err := prependUnique(ipt, c.table, c.name, rule); err != nil {
			return err
		}
	}
...
func (c *chain) teardown(ipt *iptables.IPTables) error {
	...
	entryRule := append(c.entryRule, "-j", c.name)
	for _, entryChain := range c.entryChains {
		exists, err := ipt.Exists(c.table, entryChain, entryRule...)
		if err != nil {
			return fmt.Errorf("could not check if chain %s has entry rule: %v", entryChain, err)
		}
		if exists {
			if err := ipt.Delete(c.table, entryChain, entryRule...); err != nil {
				return err
			}
		}
	}
